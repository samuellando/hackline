...
class TestFlaskHandlers(unittest.TestCase):
    ...
    def testGetRunning(self):
        ...
        d = {"start": 1, "title": "1"}
        ar.post('running/running', d.copy())
        res = self.client.get('/api/running')
    ...
    def testPostAndPutRunning(self):
        d = {"start": 1, "title": "1"}
        res = self.client.post('/api/running', data=json.dumps(d.copy()))
        self.assertEqual(res.status_code, 200)
        self.assertEqual(res.json, d)
        d = {"start": 200, "title": "2"}
        res = self.client.put('/api/running', data=json.dumps(d.copy()))
        self.assertEqual(res.status_code, 200)
        self.assertEqual(res.json, d)
    ...
    def testPutSettings(self):
        d = {"test": 1}
        res = self.client.put('/api/settings', data=json.dumps(d.copy()))
        self.assertEqual(res.status_code, 200)
        self.assertEqual(res.json, d)

    def testPatchAndPostSetting(self):
        d = {"test": 1}
        res = self.client.post('/api/setting', data=json.dumps(d.copy()))
        self.assertEqual(res.status_code, 200)
        self.assertEqual(res.json, d)
        d = {"test": 2}
        res = self.client.patch('/api/setting', data=json.dumps(d.copy()))
        self.assertEqual(res.status_code, 200)
        self.assertEqual(res.json, d)

    def testPostTimeline(self):
        i = {"title": "1", "start": 1, "end": 2}
        res = self.client.post('/api/timeline', data=json.dumps(i.copy()))
        self.assertEqual(res.status_code, 200)
        i["id"] = res.json["id"]
    ...
    def testPatchTimeline(self):
        i = {"title": "2"}
        res = self.client.patch('/api/timeline/'+'1', data=json.dumps(i.copy()))
        self.assertEqual(res.status_code, 404)
        r = ar.post('intervals', {"title": "1", "start": 1, "end": 2})
        res = self.client.patch('/api/timeline/'+r["id"], data=json.dumps(i.copy()))
        self.assertEqual(res.status_code, 200)
