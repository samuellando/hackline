...
const running = t.router({
	getRunning: protectedProcedure.query(async (opts) => {
		const { ctx } = opts;
...
const settings = t.router({
	getSettings: protectedProcedure.query(async (opts) => {
		const { ctx } = opts;
		return getSettings(ctx.user);
	}),
	setSettings: protectedProcedure.input(z.record(z.string(), z.any())).mutation(async (opts) => {
		const { ctx, input } = opts;
		return setSettings(ctx.user, input);
	}),
	setSetting: protectedProcedure.input(z.record(z.string(), z.any())).mutation(async (opts) => {
		const { ctx, input } = opts;
...
const apiKey = t.router({
	getApiKey: protectedProcedure.query(async (opts) => {
		const { ctx } = opts;
		return getApiKey(ctx.user);
	}),
	deleteApiKey: protectedProcedure.mutation(async (opts) => {
		const { ctx } = opts;
